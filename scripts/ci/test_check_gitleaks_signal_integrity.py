...
def _run(
    *, log_text: str | None, tmp_path: Path, min_commits: str = "1", range_spec: str | None = None
    ...
    return subprocess.run(args, cwd=tmp_path, text=True, capture_output=True, check=False)
...
def test_deletion_only_range_passes_despite_zero_scanned(tmp_path: Path) -> None:
    ...
    result = _run(log_text=_ZERO_COMMITS_LOG, tmp_path=tmp_path, range_spec=f"{base}..{deletion}")
...
def test_empty_range_still_fails_when_range_given(tmp_path: Path) -> None:
    ...
    result = _run(log_text=_ZERO_COMMITS_LOG, tmp_path=tmp_path, range_spec=f"{deletion}..{deletion}")
...
def test_zero_scanned_fails_when_range_adds_lines(tmp_path: Path) -> None:
    ...
    result = _run(log_text=_ZERO_COMMITS_LOG, tmp_path=tmp_path, range_spec=f"{base}..{addition}")
...
def test_unresolvable_range_fails(tmp_path: Path) -> None:
    ...
    result = _run(log_text=_ZERO_COMMITS_LOG, tmp_path=tmp_path, range_spec="nonexistent1..nonexistent2")
