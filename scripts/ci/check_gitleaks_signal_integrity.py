...
def _is_deletion_only_range(range_spec: str, min_commits: int) -> bool:
    ...
    for line in log.stdout.splitlines():
        if line.startswith("@"):
            commits += 1
        elif line and line.split("\t", 1)[0].isdigit() and int(line.split("\t", 1)[0]) > 0:
            return False
...
def main() -> int:
    ...
    parser.add_argument("--log", required=True, help="Path to the tee'd gitleaks CLI log (stdout+stderr)")
    parser.add_argument(
        "--min-commits",
        type=int,
        default=1,
    ...
    if not log_path.is_file():
        print(f"::error::gitleaks log not found at {args.log} -- the scan step may not have run at all.")
        return 1
    ...
    if commits_scanned < args.min_commits:
        if args.range_spec and _is_deletion_only_range(args.range_spec, args.min_commits):
            print(
